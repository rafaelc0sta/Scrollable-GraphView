...
@objc public class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    @IBInspectable public var barLineColor: UIColor = UIColor.darkGray

    // Fill Styles
    // ###########

    /// The background colour for the entire graph view, not just the plotted graph.
    @IBInspectable public var backgroundFillColor: UIColor = UIColor.white
    ...
    @IBInspectable public var fillGradientStartColor: UIColor = UIColor.white
    /// If fillType is set to .Gradient, then this will be the ending colour for the gradient.
    @IBInspectable public var fillGradientEndColor: UIColor = UIColor.black
    ...
    @IBInspectable public var dataPointFillColor: UIColor = UIColor.black
    ...
    public var customDataPointPath: ((_ centre: CGPoint) -> UIBezierPath)?
    ...
    public var customAnimationEasingFunction: ((_ t: Double) -> Double)?
    ...
    @IBInspectable public var referenceLineColor: UIColor = UIColor.black
    ...
    @IBInspectable public var referenceLineLabelColor: UIColor = UIColor.black
    ...
    @IBInspectable public var referenceLineNumberStyle: NumberFormatter.Style = .none
    ...
    @IBInspectable public var dataPointLabelColor: UIColor = UIColor.black
    ...
    private func setup() {
        ...
        let detectedRange = calculateRangeForEntireDataset(data: self.data)
        ...
        for i in 0 ..< data.count {
            ...
            let position = calculatePosition(index: i, value: value)
    ...
    private func updateFrames() {
        ...
        referenceLineView?.setViewport(viewportWidth: viewportWidth, viewportHeight: viewportHeight)
    ...
    private func createLinePath() -> UIBezierPath {
        ...
        zeroYPosition = calculatePosition(index: 0, value: self.range.min).y
        ...
        if(shouldFill) {
            ...
            let lastDataPoint = graphPoints[activePointsInterval.upperBound]
    ...
    private func updateLabels(_ deactivatedPoints: [Int], _ activatedPoints: [Int]) {
        ...
        for point in deactivatedPoints {
            labelPool.deactivateLabelForPointIndex(pointIndex: point)
        }

        // Grab an unused label and update it to the right position for the newly activated poitns
        for point in activatedPoints {
            let label = labelPool.activateLabelForPointIndex(pointIndex: point)
            ...
            let rangeMin = (shouldAutomaticallyDetectRange || shouldAdaptRange) ? self.range.min : self.rangeMin
            let position = calculatePosition(index: point, value: rangeMin)
    ...
    private func repositionActiveLabels() {
        for label in labelPool.activeLabels {
            ...
            let rangeMin = (shouldAutomaticallyDetectRange || shouldAdaptRange) ? self.range.min : self.rangeMin
            let position = calculatePosition(index: 0, value: rangeMin)
    ...
    private func determineDeactivatedPoints() -> [Int] {
        ...
        deactivatedPoints.subtract(currSet)
    ...
    private func determineActivatedPoints() -> [Int] {
        ...
        activatedPoints.subtract(prevSet)
  ...
  private func filterPointsForLabels(fromPoints points:[Int]) -> [Int] {
    ...
    private func startAnimations(withStaggerValue stagger: Double = 0) {
        ...
        var pointsToAnimate: Range<Int> = 0 ..< 0
    ...
    fileprivate func intervalForActivePoints() -> Range<Int> {
    ...
    fileprivate func graphPointForIndex(index: Int) -> GraphPoint {
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createPath () -> UIBezierPath {
        ...
        for i in 0...activePointsInterval.count {

            var location = CGPoint.zero


            if let pointLocation = self.graphViewDrawingDelegate?.graphPointForIndex(index: i).location {
...
private class ReferenceLineDrawingView : UIView {
    ...
    private func createReferenceLinesPath() -> UIBezierPath {
        ...
        addLineWithTag(tag: maxString, from: maxLineStart, to: maxLineEnd, inPath: referenceLinePath)
        addLineWithTag(tag: minString, from: minLineStart, to: minLineEnd, inPath: referenceLinePath)
        ...
        createIntermediateReferenceLines(rect: initialRect, numberOfIntermediateReferenceLines: self.numberOfIntermediateReferenceLines, forPath: referenceLinePath)
    ...
    private func recursiveCreateIntermediateReferenceLines(rect: CGRect, width: CGFloat, forPath path: UIBezierPath, remainingPartitions: Int) -> UIBezierPath {
        ...
        recursiveCreateIntermediateReferenceLines(rect: topRect, width: width * intermediateLineWidthMultiplier, forPath: path, remainingPartitions: remainingPartitions - 1)
        recursiveCreateIntermediateReferenceLines(rect: bottomRect, width: width * intermediateLineWidthMultiplier, forPath: path, remainingPartitions: remainingPartitions - 1)
    ...
    private func createReferenceLineFrom(from lineStart: CGPoint, to lineEnd: CGPoint, inPath path: UIBezierPath) {
        if(shouldAddLabelsToIntermediateReferenceLines) {
            ...
            let value = calculateYAxisValueForPoint(point: lineStart)
            let numberFormatter = referenceNumberFormatter()
            ...
            addLineWithTag(tag: valueString, from: lineStart, to: lineEnd, inPath: path)

        } else {
            addLineFrom(from: lineStart, to: lineEnd, inPath: path)
        }
    }

    private func addLineWithTag(tag: String, from: CGPoint, to: CGPoint, inPath path: UIBezierPath) {

        let boundingSize = boundingSizeForText(text: tag)
        let leftLabel = createLabelWithText(text: tag)
        let rightLabel = createLabelWithText(text: tag)
        ...
        addLineWithGaps(from: from, to: to, withGaps: gaps, inPath: path)
    }
    ...
    private func addLineWithGaps(from: CGPoint, to: CGPoint, withGaps gaps: [(start: CGFloat, end: CGFloat)], inPath path: UIBezierPath) {
        ...
        if(gaps.count <= 0) {
            addLineFrom(from: from, to: to, inPath: path)
        }
            ...
            addLineFrom(from: from, to: gapLeft, inPath: path)
            addLineFrom(from: gapRight, to: to, inPath: path)
        }
            ...
            addLineFrom(from: from, to: firstGapLeft, inPath: path)
            ...
            addLineFrom(from: lastGapRight, to: to, inPath: path)
        }
    }
